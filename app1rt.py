...
def calculate_raw_summary(raw_dfs: dict, sample_map: dict) -> pd.DataFrame:
    ...
    Это устраняет ложноположительные (U-ямы) и ловит ложноотрицательные (слабые).
    """
    ...
    for w in wells:
        ...
        for ch in display_channels:
            ...
            if ch in raw_dfs and w in raw_dfs[ch].columns:
                ...
                # 3. Вычисляем базу (ноль графика на чистом участке)
                baseline = smooth.min()
                ...
                # ТОПОЛОГИЯ КРИВОЙ:
                # Оцениваем шум в начале и формирование плато в конце
                ...
                # Правило 1 (Чувствительность): Прирост флуоресценции >= 8 RFU.
                # Это ловит экстремально слабые кривые (например, ROX в 6 лунке).
                # Правило 2 (Позиция): Пик должен быть после 15 цикла.
                if max_peak >= 8.0 and peak_idx > 15:
                    
                    # Правило 3 (Защита от U-ям): Кривая не должна начинаться высоко и падать.
                    # Защищает лунку 5 от ложного ROX.
                    # Правило 4 (Плато): В конце график не должен рухнуть в ноль.
                    if start_noise < (max_peak * 0.5) and final_plateau >= (max_peak * 0.3):
                        
                        # Правило 5 (Производная): Кривая должна иметь резкий подъем (S-изгиб).
                        # Отсекает ложноположительные ответы на медленном линейном дрейфе.
                        max_slope = norm.diff(periods=3).max()
                        if max_slope >= 1.0: 
                            has_signal = True
                            
                            # Вычисление Ct: пересечение 10% порога (но не ниже 5 единиц для защиты от шума)
        ...
        # Заносим лунку в таблицу, если она не пустая (имеет образец или любой сигнал)
