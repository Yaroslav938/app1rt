...
with tab1:
    ...
    if uploaded_rtf:
        ...
        # ВОССТАНОВЛЕННЫЕ ИНСТРУКЦИИ НА ПЕРВОЙ ВКЛАДКЕ
        st.markdown("---")
        col1, col2 = st.columns(2)
        with col1:
            st.markdown("""
        ...
        if raw_dfs:
            ...
            with col2:
                if view_mode == "Один образец (все каналы)":
                    ...
                    # Собираем все каналы в один датафрейм (как в бэкап 6)
                    plot_data = {}
            ...
            if not df_display.empty:
                ...
                if apply_baseline:
                    ...
                    # Явное и безопасное вычитание по столбцам (решает проблему "съехавшего ROX")
                    df_display = df_display.sub(baseline, axis=1) 

                fig = go.Figure()
                
                # Цвета как в бэкапе №6 для устранения визуальной путаницы
                colors_map = {
                    "Fam": "#2ca02c",  # Зеленый
                    "Hex": "#ffb000",  # Желтый
                    "Rox": "#ff5722",  # Оранжевый / Красный
                    "Cy5": "#d32f2f",  # Темно-красный (Purple в прошлой версии)
                ...
                if view_mode == "Один образец (все каналы)":
                    ...
                    for w in df_display.columns:
                        ...
                        fig.add_trace(go.Scatter(
                            ...
                            mode='lines',
                            line=dict(width=2), # Для одного канала лунки будут разного цвета
                            opacity=0.85,
                        ))
