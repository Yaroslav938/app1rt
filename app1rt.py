    ...
    for w in wells:
        ...
        for ch in display_channels:
            ...
            if ch in raw_dfs and w in raw_dfs[ch].columns:
                ...
                # Шаг 1: Отрезаем первые 5 циклов (там всегда сильный оптический шум и калибровка)
                valid_series = series.iloc[5:] if len(series) > 10 else series
                
                # Шаг 2: Сглаживаем кривую (скользящее среднее)
                smooth = valid_series.rolling(window=3, min_periods=1, center=True).mean()
                
                # Шаг 3: Находим абсолютный минимум на этом чистом участке и берем его за идеальный ноль
                baseline = smooth.min()
                norm = smooth - baseline
                
                max_val = norm.max()
                
                # УМНЫЕ ФИЛЬТРЫ ДЛЯ ОТСЕЧЕНИЯ АРТЕФАКТОВ (Линейный дрейф, падение, U-кривые)
                if len(norm) > 5:
                    max_slope = norm.diff().max() # Максимальный прирост флуоресценции за 1 цикл (крутизна)
                    final_val = norm.iloc[-3:].mean() # Среднее значение в самом конце реакции
                    
                    # Строгие математические условия настоящей ПЦР-кривой:
                    # 1. max_val >= 15: Общий прирост должен быть выше базового шума
                    # 2. max_slope >= 1.0: Должен быть выраженный крутой S-образный подъем (отсекает линейный дрейф)
                    # 3. final_val >= (max_val * 0.4): В конце кривая должна остаться наверху (отсекает падающие и U-кривые, которые дали ложный ROX в 5 лунке)
                    if max_val >= 15 and max_slope >= 1.0 and final_val >= (max_val * 0.4):
                        has_signal = True
                        # Ищем пересечение на 10% от максимума (но не менее 10 единиц)
                        threshold = max(max_val * 0.1, 10) 
                        cross = norm[norm > threshold]
                        if not cross.empty:
                            ct_val = round(cross.index[0], 1)
                            
            row[f"{ch} Ct"] = ct_val
            
        # Добавляем в таблицу только если лунка подписана или в ней обнаружена настоящая амплификация
