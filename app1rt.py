        ...
        if raw_dfs:
            ...
            if not available_channels:
                ...
                with col1:
                    ...
                    apply_baseline = st.checkbox("Вычесть базовую линию", value=True, help="Вычитает минимальное значение для каждой лунки, чтобы график начинался от нуля, как в родном ПО амплификатора.")
                
                if selected_ch:
                    df_plot = raw_dfs[selected_ch]
                    all_wells = df_plot.columns.tolist()
                    
                    with col2:
                        selected_wells = st.multiselect("Выберите лунки для отображения:", all_wells, default=all_wells[:4])
                        
                    if selected_wells:
                        df_display = df_plot[selected_wells]
                        if apply_baseline:
                        ...
                        with st.expander("Показать таблицу сырых значений"):
                            st.dataframe(df_plot[selected_wells], use_container_width=True)