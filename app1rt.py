...
# ─── Custom CSS (Полностью из твоего исходника) ───
st.markdown("""
<style>
...
# ═══════════════════════════════════════════════════════════════════
# УТИЛИТЫ (Оптимизированный парсинг RTF и Сырых данных)
# ═══════════════════════════════════════════════════════════════════
...
def parse_rtf_protocol(content_bytes: bytes) -> dict:
    ...
    for line in lines:
        ...
        # Динамический поиск колонок (Защита от лишних разделителей амплификатора)
        ...
        if raw_dfs:
            ...
            if not available_channels:
                ...
                if not df_display.empty:
                    ...
                    if view_mode == "Один образец (все каналы)":
                        # Привязываем физические цвета к каналам флуоресценции
                        color_map = {
                            "Fam": "#2ca02c",  # Зеленый
                        ...
                        fig = px.line(df_display, x=df_display.index, y=df_display.columns, 
                                      ...
                                      labels={"value": "Уровень флуоресценции", "Цикл": "Цикл амплификации", "variable": "Канал"})
                    else:
                        fig = px.line(df_display, x=df_display.index, y=df_display.columns,
                                      labels={"value": "Уровень флуоресценции", "Цикл": "Цикл амплификации", "variable": "Лунка"})
                    
                    # Настройки внешнего вида графика (как в профессиональном ПО)
                    fig.update_layout(
                        plot_bgcolor='white',
                        hovermode="x unified",
                    ...
                    # Заменяем st.line_chart на st.plotly_chart
