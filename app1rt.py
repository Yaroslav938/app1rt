    ...
    for line in lines:
        ...
        if sample_section and line_clean.startswith("|") and not line_clean.startswith("|*"):
            ...
            if well_idx != -1 and len(parts) > well_idx + 1:
                ...
                # Динамический расчет смещения колонок
...
def parse_r48(content_bytes: bytes) -> dict:
    """Парсер сырых файлов данных (.r48, .r96) для построения кривых амплификации."""
    text = None
    for enc in ("cp1251", "utf-8", "latin-1"):
        try:
    ...
    # Находим количество циклов ПЦР (из заголовка XCYC)
    num_cycles = 0
    ...
    if num_cycles == 0:
        num_cycles = 50 # Если не найдено, ставим дефолт
        
    channels = ['Fam', 'Hex', 'Rox', 'Cy5', 'Cy5.5']
    raw_data = {ch: [] for ch in channels}
    
    # Парсинг матрицы флуоресценции
    for line in lines:
        parts = line.strip().split()
        if len(parts) < 10: continue # Пропускаем пустые или короткие строки
        ch_raw = parts[0].split('_')[0]
        
        if ch_raw in channels:
            try:
                # В файлах ДНК-технологии значения лунок начинаются с 8-й колонки (индекс 7)
    ...
    for ch in channels:
        if len(raw_data[ch]) > 0:
            # Берем только первый блок (собственно амплификацию, отсекая производные если они есть)
            block = raw_data[ch][:num_cycles]
            num_wells = len(block[0])
            
            # Определяем размерность плашки для красивых названий (A1, B2 и т.д.)
            ...
            # Запасной вариант, если нестандартная плашка
...
def analyze_plant_gmo(protocol: dict, cfg: dict, is_swab: bool = False) -> dict:
    ...
    for s in test_samples:
        ...
        if is_swab:
            # В режиме смывов любая амплификация - это контаминация (не сравниваем с ПКО)
            # Если Ct <= 30 - считаем сильной контаминацией (+), если > 30 - следовой (мало)
            ...
            # Растение (стандартная логика пищевых продуктов)
...
with tab1:
    ...
    if uploaded:
        ...
        if protocol and not protocol["samples"]:
            ...
            if active_test == "unknown":
                ...
                if active_test == "gmo":
                    ...
                    res = analyze_plant_gmo(protocol, cfg_gmo, is_swab=is_swab_mode)
                    if "error" in res:
                        st.error(res["error"])
                    else:
                        # ── Контроли ──
                        ...
                        for sr in res["samples"]:
                            if "ОБНАРУЖЕНА КОНТАМИНАЦИЯ" in sr["conclusion"] or "ГМ растение" in sr["conclusion"] or "невалидна" in sr["conclusion"]: 
                                icon, box = "🔴", "error-box"
                            elif "Смыв чистый" in sr["conclusion"] or "не обнаружена (реакция валидна)" in sr["conclusion"] or "не обнаружены" in sr["conclusion"]: 
                            ...
                            # Расшифровка сработавших каналов флуоресценции
                            details = []
                            if sr['plant_det'] == '+':
                ...
                # ==========================================
                    ...
                    if "error" in res:
                        ...
                        # ── Биологическая справка (Курица/Индейка) ──
                        with st.expander("ℹ️ Биологическая справка: что означают каналы флуоресценции?"):
                            st.markdown("""
        ...
        # Инструкция на главной странице
        st.markdown("---")
        ...
        if raw_dfs:
            ...
            if not available_channels:
                ...
                with col1:
                    ...
                    apply_baseline = st.checkbox("Вычесть базовую линию", value=True, help="Вычитает минимальное фоновое значение, чтобы кривая начиналась от нуля.")
                
                with col2:
                    if view_mode == "Один образец (все каналы)":
                        # Берем список лунок из первого доступного канала
                        ...
                        # Собираем данные по всем каналам для одной конкретной лунки
                        plot_data = {}
                ...
                if not df_display.empty:
                    if apply_baseline:
                        df_display = df_display - df_display.min()
                        
                    # Интерактивный график
                    st.line_chart(df_display, use_container_width=True, height=450)
                    
                    with st.expander("Показать таблицу сырых значений"):
