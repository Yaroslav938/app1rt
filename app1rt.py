    ...
    for w in wells:
        ...
        for ch in display_channels:
            ...
            if ch in raw_dfs and w in raw_dfs[ch].columns:
                series = raw_dfs[ch][w]
                # Математика для оценки Ct (вычет базы 3-10 циклы)
                baseline = series.iloc[2:10].mean() if len(series) > 10 else series.min()
                norm = series - baseline
                smooth = norm.rolling(window=3, min_periods=1, center=True).mean()
                
                # Игнорируем первые 5 циклов, чтобы случайные стартовые скачки не сбивали поиск Ct
                valid_smooth = smooth.iloc[5:] if len(smooth) > 5 else smooth
                max_val = valid_smooth.max()
                
                if max_val > 25: # Порог для улавливания слабых сигналов (смывы)
                    has_signal = True
                    # Порог 10% от максимума, но не ниже уровня базового шума (15 RFU)
                    threshold = max(max_val * 0.1, 15) 
                    cross = valid_smooth[valid_smooth > threshold]
                    if not cross.empty:
                        ct_val = round(cross.index[0], 1)
                        
            row[f"{ch} Ct"] = ct_val
            
        # Добавляем в таблицу только если лунка подписана или там есть рост кривой
