...
def parse_r48(content_bytes: bytes) -> dict:
    ...
    for ch in channels:
        if len(raw_data[ch]) > 0:
            # Умный фильтр: отсекаем лишние температурные шаги прибора
            if len(raw_data[ch]) >= num_cycles * 2:
                ...
                # Выбираем блок с максимальной флуоресценцией (чтение на этапе элонгации/отжига)
...
def calculate_raw_summary(raw_dfs: dict, sample_map: dict) -> pd.DataFrame:
    ...
    Включает сложную фильтрацию топологии кривой (S-shape detector).
    Устраняет ложноположительные (U-ямы, дрейфы) и ловит ложноотрицательные (с дрейфом базовой линии вниз).
    """
    ...
    for w in wells:
        ...
        for ch in display_channels:
            ...
            if ch in raw_dfs and w in raw_dfs[ch].columns:
                ...
                # 1. Отрезаем оптические скачки старта прибора (циклы 1-5 игнорируем)
                valid_series = series.iloc[5:] if len(series) > 10 else series
                
                # 2. Сглаживаем кривую (убираем "зубчатость")
                smooth = valid_series.rolling(window=3, min_periods=1, center=True).mean()
                
                # 3. ДИНАМИЧЕСКАЯ БАЗОВАЯ ЛИНИЯ (cummin)
                # Это ключевое решение! Оно выпрямляет графики, которые сначала "падают" вниз (как ROX в 6 лунке).
                rolling_min = smooth.cummin()
                norm = smooth - rolling_min
                
                max_peak = norm.max()
                peak_idx = norm.idxmax()
                
                # 4. ТОПОЛОГИЯ КРИВОЙ (Оцениваем шум в начале и формирование плато в конце)
                final_plateau = norm.iloc[-3:].mean() if len(norm) > 3 else 0
                
                # УМНЫЕ ФИЛЬТРЫ S-ОБРАЗНОЙ КРИВОЙ ПЦР:
                # Правило 1 (Чувствительность): Прирост флуоресценции >= 15 RFU.
                # Жесткий порог отсекает шум во всех пустых лунках.
                # Правило 2 (Позиция): Пик должен быть после 15 цикла (не калибровочный выброс)
                if max_peak >= 15.0 and peak_idx > 15:
                    
                    # Правило 3 (Плато): В конце график не должен рухнуть в ноль (защита от разовых скачков)
                    if final_plateau >= (max_peak * 0.3) and final_plateau >= 10.0:
                        
                        # Правило 4 (Производная): Кривая должна иметь резкий подъем (S-изгиб).
                        # Отсекает ложноположительные ответы на медленном линейном дрейфе (спасает лунку 5).
                        max_slope = norm.diff(periods=3).max()
                        if max_slope >= 2.0: 
                            has_signal = True
                            
                            # Вычисление Ct: пересечение 10% порога (но не ниже 10 единиц для надежности)
                            threshold = max(max_peak * 0.1, 10.0)
                            cross = norm[(norm.index <= peak_idx) & (norm >= threshold)]
                            
                            # Ct не может быть поставлен раньше 5 цикла
