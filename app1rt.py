...
def calculate_raw_summary(raw_dfs: dict, sample_map: dict) -> pd.DataFrame:
    """
    Модуль анализа сырых данных флуоресценции.
    Использует топологический фильтр для защиты от артефактов прибора.
    """
    ...
    for w in wells:
        ...
        for ch in display_channels:
            ...
            if ch in raw_dfs and w in raw_dfs[ch].columns:
                ...
                # 1. ОТСЕЧЕНИЕ СТАРТОВОГО ШУМА
                # Первые 5 циклов ПЦР всегда нестабильны из-за температурной адаптации пластика.
                valid_series = series.iloc[5:] if len(series) > 10 else series
                
                # 2. СГЛАЖИВАНИЕ ДАННЫХ
                # Применяем скользящее среднее (window=3) для устранения микро-флуктуаций сигнала
                smooth = valid_series.rolling(window=3, min_periods=1, center=True).mean()
                
                # 3. ТОПОЛОГИЧЕСКИЙ ФИЛЬТР "СТАРТ-ФИНИШ"
                # Высчитываем базовый уровень в начале реакции (первые 10 рабочих циклов)
                # и финальный уровень на стадии плато (последние 5 циклов)
                start_mean = smooth.iloc[0:10].mean() if len(smooth) > 10 else smooth.iloc[0]
                end_mean = smooth.iloc[-5:].mean() if len(smooth) > 5 else smooth.iloc[-1]
                
                # ПРАВИЛО А: ИСТИННЫЙ РОСТ (ЗАЩИТА ОТ U-ОБРАЗНЫХ АРТЕФАКТОВ)
                # Если кривая упала вниз и не вернулась выше старта (лунка 5),
                # разница end_mean - start_mean будет отрицательной или близкой к нулю.
                # Мы требуем чистый прирост минимум на 8 единиц флуоресценции.
                if (end_mean - start_mean) >= 8.0:
                    
                    # 4. НОРМАЛИЗАЦИЯ И ПОИСК ПИКА
                    # Находим самую низкую точку первой половины реакции (это наш идеальный "ноль")
                    baseline = smooth.iloc[0:20].min() if len(smooth) > 20 else smooth.min()
                    norm = smooth - baseline
                    
                    max_peak = norm.max()
                    peak_idx = norm.idxmax()
                    
                    # ПРАВИЛО B: ПОЗИЦИЯ И ВЕЛИЧИНА ПИКА
                    # Пик должен быть больше 10 единиц (защита от дрейфа) и находиться
                    # строго во второй половине реакции (после 15 цикла).
                    if max_peak >= 10.0 and peak_idx > 15:
                        
                        # ПРАВИЛО C: АНАЛИЗ ПРОИЗВОДНОЙ (КРУТИЗНА РОСТА)
                        # ПЦР-кривая имеет характерный S-образный подъем.
                        # Мы ищем максимальный прирост за 3 последовательных цикла.
                        # Если он меньше 1.0, значит это не ПЦР, а медленный системный перегрев.
                        max_slope = norm.diff(periods=3).max()
                        if max_slope >= 1.0: 
                            has_signal = True
                            
                            # 5. ВЫЧИСЛЕНИЕ ПОРОГА И ПОСТАНОВКА Ct
                            # Порог пересечения равен 10% от пика (динамическая подстройка).
                            # Но порог не может быть опущен ниже 5 RFU во избежание захвата шума.
                            threshold = max(max_peak * 0.1, 5.0)
                            cross = norm[(norm.index <= peak_idx) & (norm >= threshold)]
                            
                            # Точка Ct (Threshold Cycle) не может лежать раньше 5-го цикла
        ...
        # Лунка выводится, если в ней обнаружен достоверный сигнал или она подписана врачом
...
with tab2:
    ...
    uploaded_raw = st.file_uploader("📂 Загрузите файл сырых данных (.r48, .r96)", type=["r48", "r96", "txt"])
        ...
        if raw_dfs:
            ...
            if not available_channels:
                ...
                with col1:
                    ...
                    apply_baseline = st.checkbox("Вычесть базовую линию", value=True, help="Вычитает средний фоновый шум первых циклов (как это делает софт амплификатора).")
                ...
                if not df_display.empty:
                    # 1. Сглаживание (делаем ДО базы, чтобы сгладить стартовые артефакты)
                    if apply_smoothing:
                        df_display = df_display.rolling(window=5, min_periods=1, center=True).mean()
                        
                    # 2. Вычитание базовой линии
                    if apply_baseline:
                        # Используем умную логику: отрезаем шумные первые 5 циклов для поиска нуля
                        safe_min = df_display.iloc[5:].min() if len(df_display) > 10 else df_display.min()
                        df_display = df_display - safe_min
                        
                    # Используем Plotly для красивой и правильной отрисовки
                    if view_mode == "Один образец (все каналы)":
                        color_map = {
                            "Fam": "#2ca02c",  # Зеленый
