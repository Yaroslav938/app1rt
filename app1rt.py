    ...
    for w in wells:
        ...
        for ch in display_channels:
            ...
            if ch in raw_dfs and w in raw_dfs[ch].columns:
                ...
                # Шаг 1: Отрезаем первые 5 циклов (сильный оптический шум и калибровка)
                ...
                # Шаг 3: УМНЫЙ ФИЛЬТР БАЗЫ (через кумулятивный минимум)
                # Это идеально отсекает "U-образные" провалы флуоресценции
                rolling_min = smooth.cummin()
                rises = smooth - rolling_min
                
                max_rise = rises.max()
                max_rise_idx = rises.idxmax()
                
                # Порог 15 единиц чистого прироста (очень высокая чувствительность)
                if max_rise >= 15: 
                    # Настоящий старт амплификации (база) для этого пика
                    baseline = rolling_min[max_rise_idx]
                    norm = smooth - baseline
                    
                    final_val = norm.iloc[-3:].mean()
                    
                    # Находим максимальную скорость роста за 3 цикла (производная)
                    max_slope = norm.diff(periods=3).max()
                    
                    # ФИЛЬТРЫ АРТЕФАКТОВ:
                    # 1. final_val: В конце кривая не должна упасть (защита от разовых скачков)
                    # 2. max_slope >= 3.0: Кривая должна иметь "крутой" участок (защита от линейного дрейфа)
                    if (final_val >= max_rise * 0.15 or final_val >= 10) and max_slope >= 3.0:
                        has_signal = True
                        # Ищем пересечение (10% от максимума, но не ниже 8 RFU)
                        threshold = max(max_rise * 0.1, 8) 
                        cross = norm[(norm.index <= max_rise_idx) & (norm >= threshold)]
                        if not cross.empty:
