...
def parse_rtf_protocol(content_bytes: bytes) -> dict:
    ...
    # 1. Умная декодировка (Защита от ошибки Windows CP1251 и кракозябр)
    for enc in ("utf-8", "cp1251", "latin-1"):
        try:
            temp_raw = content_bytes.decode(enc)
            if "Номер" in temp_raw or "Fam" in temp_raw or "Cp" in temp_raw or "A1" in temp_raw:
                raw = temp_raw
                break
    ...
    if raw is None:
        raw = content_bytes.decode("cp1251", errors="ignore")

    # 2. Очистка от RTF тегов
    ...
    # Используем splitlines(), чтобы избежать проблем с переносами строк Mac/Linux/Windows
    ...
    for line in lines:
        ...
        if "Дата:" in line_clean:
            m = re.search(r'Дата:\s*\|*([^|]+)', line_clean)
            if m: result["date"] = m.group(1).strip()
        elif "Оператор:" in line_clean:
            m = re.search(r'Оператор:\s*\|*([^|]+)', line_clean)
            if m: result["operator"] = m.group(1).strip()
        elif "Номер протокола:" in line_clean:
            m = re.search(r'Номер протокола:\s*\|*([^|]+)', line_clean)
            if m: result["protocol_number"] = m.group(1).strip()
        elif "Тест:" in line_clean:
            m = re.search(r'Тест:\s*\|*([^|]+)', line_clean)
            if m: result["test_name"] = m.group(1).strip()
        elif "Программа амплификации:" in line_clean:
            m = re.search(r'Программа амплификации:\s*\|*([^|]+)', line_clean)
            if m: result["amplification_program"] = m.group(1).strip()

        # Динамический поиск колонок (находит Fam, Hex, Rox независимо от их позиции)
        p_low = line_clean.lower()
        if "номер лунки" in p_low and ("идентификатор" in p_low or "пробирка" in p_low or "образец" in p_low):
            sample_section = True
            sep = "|" if "|" in line_clean else "\t"
            parts = [p.strip() for p in line_clean.split(sep)]
            for idx, p in enumerate(parts):
                p_low_h = p.lower()
                if "номер лунки" in p_low_h: well_header_idx = idx
                elif "fam" in p_low_h or "green" in p_low_h: headers_map["Fam"] = idx
                elif "hex" in p_low_h or "yellow" in p_low_h: headers_map["Hex"] = idx
                elif "rox" in p_low_h or "orange" in p_low_h: headers_map["Rox"] = idx
                elif "cy5.5" in p_low_h: headers_map["Cy5.5"] = idx
                elif "cy5" in p_low_h or "red" in p_low_h: headers_map["Cy5"] = idx
            continue

        # Чтение строк с результатами по каждой лунке
        if sample_section and not line_clean.startswith(sep+"*"):
            parts = [p.strip() for p in line_clean.split(sep)]
        ...
        # Конец таблицы
        if sample_section and (sep+"*") in line_clean:
            break
...
def calculate_raw_summary(raw_dfs: dict, protocol_data: dict = None) -> pd.DataFrame:
    """
    Создает чистую итоговую таблицу Ct для вкладки сырых данных.
    СИНХРОНИЗАЦИЯ: Если загружен RTF, берет точные данные из него.
    Если нет - использует адаптивный алгоритм сигнал/шум (SNR).
    """
    ...
    # 1. Создаем карту эталонных значений из RTF (если загружен)
    rtf_map = {}
    ...
    for w in wells:
        ...
        for ch in display_channels:
            ...
            # --- ПРИОРИТЕТ 1: Берем 100% точное значение из RTF (если он есть) ---
            if protocol_data and "samples" in protocol_data:
                if w in rtf_map:
            ...
            # --- ПРИОРИТЕТ 2: Математический расчет (только если нет RTF) ---
            else:
                if ch in raw_dfs and w in raw_dfs[ch].columns:
                    ...
                    if len(series) > 15:
                        ...
                        # Базовый шум вычисляем на стабильном отрезке (5-15 циклы)
                        base_region = smooth.loc[5:15]
                        baseline = base_region.mean()
                        
                        # Высчитываем АМПЛИТУДУ ШУМА (разброс базовой линии)
                        ...
                        # ФИЛЬТР "СИГНАЛ-ШУМ" (SNR):
                        # Пик должен быть больше 10 единиц И превышать фоновый шум в 3 раза!
                        # Это отлично отсекает шумные каналы вроде FAM.
                        if max_peak >= 10.0 and max_peak > (noise_amp * 3.0) and peak_idx > 15:
                            
                            max_slope = norm.diff(periods=2).max()
                            final_val = norm.iloc[-3:].mean()
                            
                            # Проверка на крутизну и плато
            ...
            # ЗАЩИТА ОТ КРАША PYARROW: 
            # Переводим ВСЕ значения строго в текст, чтобы избежать смешения типов
...
# Проверка библиотек (подсказка для разработчика)
if rtf_to_text is None:
    st.sidebar.warning("⚠️ Библиотека 'striprtf' не найдена! Если парсинг не работает, выполните команду: `pip install striprtf`")
    ...
    if uploaded_raw:
        ...
        if raw_dfs:
            ...
            if not available_channels:
                ...
                # ── ИТОГОВАЯ ТАБЛИЦА (Синхронизация с RTF + Расчет из сырых данных) ──
                st.markdown("---")
                st.markdown("#### 📊 Итоговая таблица значений Ct")
                
                # Если протокол RTF был загружен на 1-й вкладке, уведомляем пользователя
                if protocol_data and "samples" in protocol_data:
                    st.success("✅ Значения Ct в таблице ниже полностью синхронизированы с оригинальным RTF-протоколом прибора.")
                else:
                    st.warning("⚠️ RTF-протокол не загружен. Значения Ct рассчитаны программно из сырых кривых и могут отличаться от прибора.")
