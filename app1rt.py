...
def calculate_raw_summary(raw_dfs: dict, sample_map: dict) -> pd.DataFrame:
    """Создает итоговую таблицу со значениями Max флуоресценции и оценкой Ct из сырых данных"""
    if not raw_dfs: return pd.DataFrame()
    first_ch = list(raw_dfs.keys())[0]
    wells = raw_dfs[first_ch].columns.tolist()
    
    rows = []
    # Фиксированный порядок каналов для красивого отображения
    display_channels = ["Fam", "Hex", "Rox", "Cy5", "Cy5.5"]
    
    for w in wells:
        row = {"Лунка": w, "Образец": sample_map.get(w, "—")}
        is_empty = True # Флаг для отсечения полностью пустых лунок
        
        for ch in display_channels:
            if ch in raw_dfs and w in raw_dfs[ch].columns:
                series = raw_dfs[ch][w]
                
                # Математика как в амплификаторе: вычет базы (циклы 3-10) и сглаживание
                baseline = series.iloc[2:10].mean() if len(series) > 10 else series.min()
                norm = series - baseline
                smooth = norm.rolling(window=5, min_periods=1, center=True).mean()
                
                max_val = smooth.max()
                if max_val > 50: 
                    is_empty = False
                    
                # Оценка Ct (пересечение порога 10% от максимума)
                ct = "—"
                if max_val > 150: # Если сигнал выше фонового шума
                    threshold = max_val * 0.1
                    cross = smooth[smooth > threshold]
                    if not cross.empty:
                        ct = round(cross.index[0], 1)
                        
                row[f"{ch} Ct (оценка)"] = ct
                row[f"{ch} Max RFU"] = int(max_val) if pd.notna(max_val) else 0
                
        # Добавляем в таблицу, если в лунке был хоть какой-то сигнал или это известный образец
        if not is_empty or row["Образец"] != "—":
            rows.append(row)
        ...
        if raw_dfs:
            ...
            if not available_channels:
                ...
                if not df_display.empty:
                    # 1. Вычитание базовой линии (убирает прыжки на начальных циклах)
                    if apply_baseline:
                        baseline = df_display.iloc[2:10].mean() if len(df_display) > 10 else df_display.min()
                        df_display = df_display - baseline
                        
                    # 2. Сглаживание (устраняет зубчатость)
                    if apply_smoothing:
