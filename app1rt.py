...
# ═══════════════════════════════════════════════════════════════════
# УТИЛИТЫ (Парсинг RTF и Сырых данных)
# ═══════════════════════════════════════════════════════════════════
...
def parse_r48(content_bytes: bytes) -> dict:
    ...
    for ch in channels:
        if len(raw_data[ch]) > 0:
            # Умный фильтр: устранение "зубчатости" (отсекаем лишние температурные шаги)
            if len(raw_data[ch]) >= num_cycles * 2:
                ...
                # Выбираем блок с максимальной флуоресценцией (чтение на 60 градусах)
...
def calculate_raw_summary(raw_dfs: dict, sample_map: dict) -> pd.DataFrame:
    """Создает классическую чистую итоговую таблицу Ct из сырых данных"""
    ...
    for w in wells:
        ...
        for ch in display_channels:
            ...
            if ch in raw_dfs and w in raw_dfs[ch].columns:
                ...
                # Шаг 1: Отрезаем первые 3 цикла (там всегда системный шум калибровки)
                valid_series = series.iloc[3:] if len(series) > 10 else series
                
                # Шаг 2: Сглаживаем кривую для поиска общих трендов (S-формы)
                smooth = valid_series.rolling(window=3, min_periods=1, center=True).mean()
                
                # Шаг 3: Находим абсолютный минимум (это наш ноль)
                baseline = smooth.min()
                ...
                # ТОПОЛОГИЯ КРИВОЙ:
                # Берем среднее значение шума за первые циклы амплификации (5-15)
                start_noise = norm.loc[5:15].mean() if len(norm) > 15 else 0
                # Берем среднее значение за последние 5 циклов (формирование плато)
                final_plateau = norm.iloc[-5:].mean() if len(norm) > 5 else 0
                
                # УМНЫЕ ФИЛЬТРЫ S-ОБРАЗНОЙ КРИВОЙ ПЦР:
                # 1. max_peak >= 5.0: Ультра-высокая чувствительность для улавливания слабых смывов (как в лунке 6)
                # 2. start_noise < max_peak * 0.4: Кривая должна начинаться "внизу" (отсекает U-образный артефакт лунки 5)
                # 3. final_plateau >= max_peak * 0.3: Кривая должна остаться "наверху" к концу реакции
                # 4. peak_idx > 12: Пик должен быть сформирован в основной/поздней фазе ПЦР
                if max_peak >= 5.0 and start_noise < (max_peak * 0.4) and final_plateau >= (max_peak * 0.3) and peak_idx > 12:
                    has_signal = True
                    
                    # Порог пересечения: 10% от пика, но не меньше 2.0 единиц флуоресценции
                    threshold = max(max_peak * 0.1, 2.0)
                    cross = norm[(norm.index > 5) & (norm >= threshold)]
                    if not cross.empty:
                        ct_val = round(cross.index[0], 1)
                            
            row[f"{ch} Ct"] = ct_val
            
        # Добавляем в таблицу только если лунка подписана или в ней обнаружена амплификация
...
# ═══════════════════════════════════════════════════════════════════
# АНАЛИЗАТОРЫ (Логика в точности из файлов Excel)
# ═══════════════════════════════════════════════════════════════════
...
# Глобальная переменная для связи вкладок
protocol_data = None
        ...
        if raw_dfs:
            ...
            if not available_channels:
                ...
                if not df_display.empty:
                    ...
                    if apply_baseline:
                        # Используем ту же умную логику: отрезаем шумные первые 5 циклов для поиска нуля
