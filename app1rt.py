    ...
    for w in wells:
        ...
        for ch in display_channels:
            ...
            if ch in raw_dfs and w in raw_dfs[ch].columns:
                ...
                # Шаг 1: Отрезаем первые 5 циклов (там всегда сильный шум и калибровка)
                ...
                # Шаг 3: Находим абсолютный минимум на этом чистом участке и берем его за ноль
                baseline = smooth.min()
                norm = smooth - baseline
                max_val = norm.max()
                
                # Порог 15 единиц чистого прироста флуоресценции (очень высокая чувствительность)
                if max_val >= 15: 
                    has_signal = True
                    # Порог 10% от максимума, но не ниже 10 RFU (защита от микро-шумов)
        ...
        # Добавляем в таблицу только если лунка подписана или там есть любой рост
